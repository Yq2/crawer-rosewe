...
const homeUrl='https://www.modlily.com/';
let $headers = {
...
function modlily() {
    ...
    function parseCategory(body,callback) {
        ...
        $("ul.catehome li").each( function(index,element) {
            if (index!==0) {
                let categoryItem={};
                let parent_url=homeUrl+$(this).find("a.nav_t").attr("href");
                categoryItem.url=parent_url;
                categoryItem.name=$(this).find("a.nav_t").text().trim();
                let secondCategorys=[];
        ...
        for (let i=0;i<list_category.length;i++) {
            ...
            for (let j=0;j<categorys.length;j++) {
                if (list_category[i].url===categorys[j].url) {
                    if (categorys[j].sencodCategoryItems.length!==0) {
    ...
    this.run=function () {
        async.waterfall([
            ...
            function (cb) {
                async.eachLimit(requestCategorys , 1 ,function (category,callback) {
                    async.waterfall([
                        function (fn) {
    ...
    function parseDetailPage(site,body,callback) {
        ...
        if ('30') {
            $("div#js_n_thumbImg").find("li").each(function (index,ele) {
                let imgItem={};
                imgItem.type=30;
        ...
        item.prop=[];
        $("div.pro_property table").find("tr").each(function (index) {
            if (index===0) {
        ...
        async.waterfall([
            function (cb) {
                let dynamic='https://www.modlily.com/dynamic.php';
                let headers=Function.obj_copy($headers);
    ...
    function parseList(category,body,callback) {
        ...
        let pageTotal=$("div[class='fr']").find("p").find("a").length;
        ...
        async.waterfall([
            ...
            if (err) console.log(err);
            let nextHref='';
            $("div[class='pages tc']").find("p").find("a").each(function (index,ele) {
                if ($(this).text()==='Next»') {
                    nextHref=homeUrl+$(this).attr("href");
                }
            });
            console.log('currentPage=',currentPage);
            console.log('pageTotal=',category.pageTotal);
            ...
            if (nextHref) {
                ...
                console.log(items.slice(0,5),items.slice(-5,-1));
                //items.length=0;
